...
pub async fn auth_request(mut stream: &mut TcpStream, method: AuthMethod) -> Result<()> {
    let buf = [consts::SOCKS5_VERSION, 1u8, method as u8];
    stream.write_all(&buf).await?;
    Ok(())
}

// Read auth response from server
pub async fn auth_response(mut stream: &mut TcpStream) -> Result<AuthResponse> {
    let mut buf = [0u8, 0u8];
